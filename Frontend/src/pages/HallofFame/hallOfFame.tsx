    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      {selectedSemester ? (
        <section className="rounded-[32px] border border-white/10 bg-zinc-900/80 p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-zinc-400">{selectedSemester.description}</p>

          <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {selectedSemester.panelists.map((panelist) => (
              <div key={`${panelist.name}-${panelist.role}`} className="rounded-[24px] border border-white/10 bg-black/30 p-6">
